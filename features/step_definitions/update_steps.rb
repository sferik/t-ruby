When /^I update my status using an authenticated profile$/ do
  ...
  steps %(When I run `t update 'Testing #{rand(2**100)}' --profile #{File.expand_path('../../fixtures/.trc', __FILE__)}`)
end

When /^I update my status using an unauthenticated profile$/ do
  steps %(When I run `t update 'Testing #{rand(2**100)}' --profile /tmp/.trc`)
end
