 ...
 7505382  Jul 16  2007  Jul  8 10:29       7890       3755     118        212...
  ...
  describe '#timeline' do
    before do
      ...
      stub_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '20', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'}).to_return(:body => fixture('statuses.json'))
    end
    it 'requests the correct resource' do
      @list.timeline('presidents')
      expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '20', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'})).to have_been_made
    end
    it 'has the correct output' do
      @list.timeline('presidents')
      expect($stdout.string).to eq <<-eos
...
244099460672679938,2012-09-07 15:47:01 +0000,dwiskus,"Gentlemen, you can't fight in here! This is the war room! http://t.co/kMxMYyqF"
    ...
    context '--decode-uris' do
      before do
        ...
        stub_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '20', :slug => 'presidents', :include_entities => 'true', :trim_user => 'true'}).to_return(:body => fixture('statuses.json'))
      end
      it 'requests the correct resource' do
        @list.timeline('presidents')
        expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '20', :slug => 'presidents', :include_entities => 'true', :trim_user => 'true'})).to have_been_made
      end
      it 'decodes URLs' do
        @list.timeline('presidents')
...
4611686018427387904  Sep  7 08:35  @mutgoff          Happy Birthday @imdane. ...
    ...
    context '--number' do
      before do
        stub_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '1', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'}).to_return(:body => fixture('statuses.json'))
        stub_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '200', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'}).to_return(:body => fixture('200_statuses.json'))
        stub_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '1', :max_id => '265500541700956160', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'}).to_return(:body => fixture('statuses.json'))
      end
      it 'limits the number of results to 1' do
        @list.options = @list.options.merge('number' => 1)
        @list.timeline('presidents')
        expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '1', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'})).to have_been_made
      end
      it 'limits the number of results to 201' do
        @list.options = @list.options.merge('number' => 201)
        @list.timeline('presidents')
        expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '200', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'})).to have_been_made
        expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_screen_name => 'testcli', :count => '1', :max_id => '265500541700956160', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'})).to have_been_made
      end
    end
    context 'with a user passed' do
      it 'requests the correct resource' do
      ...
      context '--id' do
        before do
          @list.options = @list.options.merge('id' => true)
          stub_get('/1.1/lists/statuses.json').with(:query => {:owner_id => '7505382', :count => '20', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'}).to_return(:body => fixture('statuses.json'))
        end
        it 'requests the correct resource' do
          @list.timeline('7505382/presidents')
          expect(a_get('/1.1/lists/statuses.json').with(:query => {:owner_id => '7505382', :count => '20', :slug => 'presidents', :include_entities => 'false', :trim_user => 'true'})).to have_been_made
        end
      end
    end
  end
